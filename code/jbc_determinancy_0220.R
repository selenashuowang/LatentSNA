...
jbc_determinancy_0220<- function(X, Y,W, H, K=2,D=1,theoretical.str,
  ...
  for (s in 1:(nscan + burn))
  {
    ...
    if (K > 0)
    {
      U<-rU_b(Fl,U,Theta, Stheta, Sutheta, Su, s2, offset=sapply(1:length(EFl), function(x) EFl[[x]]-U[,,x]%*%t(U[,,x]), simplify = FALSE))
       ...
       if(K < D){
         if(s ==1){
           a=cancor(x=Theta, y=t(U[1,,]), xcenter = FALSE, ycenter = FALSE)

           U_target=a$scores$X}else{
             for (v in 1:V){
      ...
      if(K > D){
        if(s ==1){
          a=cancor(x=Theta[,1:2], y=t(U[1,,]), xcenter = FALSE, ycenter = FALSE)
