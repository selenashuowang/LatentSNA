...
jbc_scaleAlpha_new_abcd_newmodel<- function(X, Y,W, H,K=2, D=1,theoretical.str,
                    ...
                    prior=list())
{
  ...
  input<-list(D=D, K=K,theoretical.str,nscan=nscan, burn=burn, odens=odens, prior=prior, indices = indices, indices_irt = indices_irt)
  # set random seed
  set.seed(seed)
  ...
  Ul<-sapply(1:length(X), function(x) (svd(tmp[[x]])$u[,1,drop=FALSE]%*%diag(sqrt(svd(tmp[[x]])$d[1]),nrow=1)), simplify = FALSE)
  ...
  {  
    sE<-svd(E.s)
    S<-sE$u[,1:K,drop=FALSE]%*%diag(sqrt(sE$d[1:K]),nrow=K)
  ...
  if(is.null(prior$Sutheta0)){ prior$Sutheta0<-cor(S) }
...
#   colnames(TAC) <- names_i
  ...
  for (s in 1:(nscan + burn))
  {
    ...
    if(!is.null(W)){EFl<-sapply(1:length(X), function(x) (as.numeric(a[x,]) + as.numeric(W[x,] %*% beta) + U[,,x] %*% t(U[,,x]) + S %*% t(S)), simplify = FALSE)}
    if(is.null(W)){EFl<-sapply(1:length(X), function(x) (as.numeric(a[x,]) + U[,,x] %*% t(U[,,x]) + S %*% t(S)), simplify = FALSE)}
    ...
    U<-rU_b_updated_abcd_newmodel(Fl,EFl,U,Theta, Stheta, Sutheta, Su,  s2)
    

    U[,1,]=t(scale(t(U[,1,]), scale=FALSE))
    
    tmp <- rSu(S, Su0=prior$Sutheta0.s,etau=prior$etautheta.s) 
    Ss=tmp$Su
    
    # update U,V
    if (K > 0)
    {
      S<-rU_con(Fl,S, Ss, s2, offset=sapply(EFl, function(x) x-S%*%t(S), simplify = FALSE))
      S <- S -   colMeans(S)
      
      if(s ==1){S_target<-S}
      
      if(s>1){
        tmp.s <- Procrustes(S, S_target,
                          translate = FALSE,
                          dilate = FALSE,
                          sumsq = FALSE)
        S<-tmp.s$X.new
...
#     # 
  ...
  if(!is.null(W)){EFlPM<-sapply(1:length(X), function(x) (APM[x] + as.numeric(W[x,] %*% BETAPM) + UVPM.l[[x]]  + SM %*% t(SM)), simplify = FALSE)}
  if(is.null(W)){EFlPM<-sapply(1:length(X), function(x) (APM[x] + UVPM.l[[x]]+ SM %*% t(SM)), simplify = FALSE)}
  ...
  fit <- list(BETAPM=BETAPM,GAMMAPM=GAMMAPM, COV=COV, SPM=SPM, SM = SM, S=S, APM=APM,bPM=bPM,U=U,UVPM.l=UVPM.l,THETAPM = THETAPM,UPM=UPM, ALPHAPM = ALPHAPM,EFlPM=EFlPM,ETPM=ETPM,
