...
jbc_determinancy_0221<- function(X, Y,W, H, K=2,D=1,theoretical.str,
...
{ 
  ...
  input<-list(K=K,D=D, theoretical.str,nscan=nscan, burn=burn, odens=odens, prior=prior, indices = indices, indices_irt = indices_irt)
  # set random seed
  set.seed(seed)
  ...
  VC<-matrix(nrow=0,ncol=2+length(c(seq(1,(K)*(K+1)/2), seq(1,(D)*(D+1)/2)))) 
  
  COV<-matrix(nrow = 0, ncol = K*D)
  ...
  { 
    ...
    for (v in 1:V){
      ## update variances need to update node by node
      
      Theta <-rTheta_b(Tm  , b, t(Alpha), t(Theta), t(U[v,,]), Stheta, Sutheta, Su, s1)
      
      tmp <- rSu(data.matrix(cbind(t(U[v,,]),Theta)), Su0=prior$Sutheta0,etau=prior$etautheta) 
      ...
      # update Theta update node by node
    ...
    if (K > 0)
    {
      U<-rU_b(Fl,U,Theta, Stheta, Sutheta, Su, s2, offset=sapply(1:length(EFl), function(x) EFl[[x]]-U[,,x]%*%t(U[,,x]), simplify = FALSE))
      ...
      if((s==1) & (K <D)){
        tmp_a=candisc::cancor(x=Theta, y=tmp.target, xcenter = FALSE, ycenter = FALSE)
        U_target=tmp_a$scores$Y
      }
      if((s==1) & (K >D)){
        tmp_a=candisc::cancor(x=Theta, y=tmp.target, xcenter = FALSE, ycenter = FALSE)
        U_target=cbind(tmp_a$scores$Y,tmp.target[1,(D+1):(K),])
      }
      if(s>1){
        for (v in 1:V){
          tmp <- Procrustes(t(U[v,,]), U_target,
                            translate = FALSE,
                            dilate = FALSE,
                            sumsq = FALSE)
          U[v,,]<-t(tmp$X.new)
    ...
    # Not the same as updating node by node
    # Sutheta_1 <-do.call(cbind, replicate(V, Sutheta, simplify=FALSE))
    # Su_1 <-do.call(adiag, replicate(V, Su, simplify=FALSE))
    # Stheta_1 <-Stheta
    # 
    # 
    # U.mat=matrix(NA, nrow=N, ncol=V*K)
    # 
    # for(i in 1:N){
    #   tmp.vec=c()
    #   for(v in 1:V){
    #     tmp.vec=c(tmp.vec,U[v,,i])
    #   }
    #   U.mat[i,]=tmp.vec
    # }
    # 
    # # update Theta update node by node
    # Theta_option1 <-rTheta_b(ET, b, t(Alpha), t(Theta_0),U.mat, Stheta_1, Sutheta_1, Su_1, s1)
    # 
    
    # Theta <- Theta -   colMeans(Theta)
    # 
    # 
    # if(s ==1){Theta_target<-Theta}
    # 
    # if(s>1){
    ...
    # update U, update subject by subject, within each function update by node 
    
    # tmp.Ul=sapply(1:nrow(Theta), function(i) rU(Fl,U[,,i],Theta, Stheta, Sutheta, Su, s2, offset=sapply(1:length(Fl), function(x) EFl[[x]]-U[,,x]%*%t(U[,,x]), simplify = FALSE)), simplify = FALSE)
    # tmp.Ul_1=sapply(1:nrow(Theta), function(i) tmp.Ul[[i]] -   colMeans(tmp.Ul[[i]]), simplify = FALSE)
    # 
    # U=simplify2array(tmp.Ul_1)
    

    # for( i in 1:nrow(Theta)){
    #   # update U update node by node
    #   if (K > 0)
    #   {
    #     U[,,i]<-rU(Fl,U[,,i],Theta, Stheta, Sutheta, Su, s2, offset=sapply(1:length(Fl), function(x) EFl[[x]]-U[,,x]%*%t(U[,,x]), simplify = FALSE))
    # 
    #     U[,,i] <- U[,,i] -   colMeans(U[,,i])
    # 
    #   }
    # }
