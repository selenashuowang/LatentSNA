...
jbc_scaleAlpha_new_abcd_1<- function(X, Y,W, H, D=1,theoretical.str,
...
#   colnames(TAC) <- names_i
  ...
  for (s in 1:(nscan + burn))
  {
    ...
    U[,1,]<-t(rU_b_updated_abcd(Fl,U,Theta, Stheta, Sutheta, Su, S, s2, offset=sapply(1:length(EFl), function(x) EFl[[x]]-U[,,x]%*%t(U[,,x]), simplify = FALSE)))
