...
rU_b_updated_abcd_parallel<-function(Fl,EFl, U,Theta, Stheta, Sutheta, Su,  s2)
{
  ...
  #clusterExport(cl = cl, varlist = c('Fl','EFl','U','Theta', 'Stheta', 'Sutheta', 'Su',  's2'), envir=environment())
  ...
  if(ncol(Theta)==1){utheta.tmp.list=Theta}
  if(ncol(Theta)>1){utheta.tmp.list= data.matrix(sapply( 1:N, function(n) c(Theta[n,]) , simplify = TRUE))}
  
  startTime <- Sys.time()
  
  tmp.iu=lapply( 1:V, function(u) rU_each_b_updated_abcd_3(u,  tmp.fz.new[,u],t(utheta.tmp.list),U,Theta,  Stheta, Sutheta=Sutheta[,u], Su=Su[u,u], to, K,N))
  endTime <- Sys.time()
  ...
  tmp.iu=mclapply( 1:V, function(u) rU_each_b_updated_abcd_3(u,  tmp.fz.new[,u],t(utheta.tmp.list),U,Theta,  Stheta, Sutheta=Sutheta[,u], Su=Su[u,u], to, K,N))
  endTime <- Sys.time()
  ...
  U[,1,]=t(mclapply(1:V, function(u) rU_each_b_updated_abcd_3(u,  tmp.fz.new[,u],t(utheta.tmp.list),U,Theta,  Stheta, Sutheta=Sutheta[,u], Su=Su[u,u], to, K,N), simplify = TRUE))
  endTime <- Sys.time()
  ...
  print("fourth parallel")
    if(D==1){
      startTime <- Sys.time()
      
      U[,1,]=t(parSapply(cl, 1:V, function(u) rU_each_b_updated_abcd_3(u,  tmp.fz.new[,u],t(utheta.tmp.list),U,Theta,  Stheta, Sutheta=Sutheta[,u], Su=Su[u,u], to, K,N), simplify = TRUE))
      endTime <- Sys.time()
    ...
    if(D>1){
      U[,1,]=t(parSapply(cl, 1:V, function(u) rU_each_b_updated_abcd_3(u,  tmp.fz.new[,u],utheta.tmp.list,U,Theta,  Stheta, Sutheta=Sutheta[,u], Su=Su[u,u], to, K,N), simplify = TRUE))
    }
  print("finished")
  stopCluster(cl)
