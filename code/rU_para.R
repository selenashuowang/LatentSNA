...
#'
#' @export rU
rU_para<-function(u, Fl,EFl, U,Theta, Stheta, Sutheta, Su,  s2)
{
  ...
  system.time(tmp.fz.new=parSapply(cl,1:V, function(u) sapply(1:N, function(n) sum(U[,,n]*Es[[n]][u,],na.rm=TRUE)-  U[u,,n]*Es[[n]][u,u] , simplify = TRUE),simplify = TRUE))
  stopCluster(cl)
  ...
  if(D==1){
    system.time(U[,1,]=t(sapply(1:V, function(u) rU_each_b_updated_abcd_3(u,  tmp.fz.new[,u],t(utheta.tmp.list),U,Theta,  Stheta, Sutheta=Sutheta[,u], Su=Su[u,u], to, K,N), simplify = TRUE)))
  }
  if(D>1){
  ...
  if(D==1){
    system.time(U[,1,]=t(parSapply(cl,1:V, function(u) rU_each_b_updated_abcd_3(u,  tmp.fz.new[,u],t(utheta.tmp.list),U,Theta,  Stheta, Sutheta=Sutheta[,u], Su=Su[u,u], to, K,N), simplify = TRUE)))
  }
  if(D>1){
