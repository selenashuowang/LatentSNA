...
library(parallel)
rU_b_updated_abcd_pred<-function(Fl,EFl, U,Theta, Stheta, Sutheta, Su,  s2)
{
  ...
  #tmp.new=matrix(c(U[-u,1,n],Theta[n,]))
    ...
    if(D==1){
      U[,1,]=t(sapply(1:V, function(u) rU_each_b_updated_abcd_3_pred(u,t(utheta.tmp.list),U,Theta,  Stheta, Sutheta=Sutheta[,u], Su=Su[u,u], to, K,N), simplify = TRUE))
    }
    if(D>1){
      U[,1,]=t(sapply(1:V, function(u) rU_each_b_updated_abcd_3_pred(u,utheta.tmp.list,U,Theta,  Stheta, Sutheta=Sutheta[,u], Su=Su[u,u], to, K,N), simplify = TRUE))
    }
