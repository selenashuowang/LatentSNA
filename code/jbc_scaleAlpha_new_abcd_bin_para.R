...
jbc_scaleAlpha_new_abcd_para_bin<- function(X, Y,W, H, D=1,theoretical.str,family_behavior,
                    ...
                    prior=list())
{
  ...
  Nchunk_list=split(1:N,ceiling(seq_along(1:N) / 100))
  ...
  s2<-mean(sapply(Fl, function(x) mean(x^2, na.rm=TRUE)), na.rm=TRUE)
  

  #s2<-mean(parSapply(cl,1:length(X), function(x) mean((Fl[[x]] - a[x,])^2, na.rm=TRUE)), na.rm=TRUE)
 ...
 # s2<-mean(parSapply(cl,1:length(X), function(x) mean((Fl[[x]] - a[x,])^2, na.rm=TRUE)), na.rm=TRUE)
  ...
  # system.time(Ul <- parSapply(cl, Nchunk_list[1:2], function(a) sapply(a, function(x) (svd(tmp[[x]])$u[,1:K,drop=FALSE]%*%diag(sqrt(svd(tmp[[x]])$d[1:K]),nrow=K)), simplify = FALSE)))
  # stopCluster(cl)
  # # 
  # system.time(Ul<-sapply(Nchunk_list, function(a) sapply(a, function(x) (svd(tmp[[x]])$u[,1:K,drop=FALSE]%*%diag(sqrt(svd(tmp[[x]])$d[1:K]),nrow=K)), simplify = FALSE)))
  # 
  #system.time(Ul <- sapply(1:length(X), function(x) (svd(tmp[[x]])$u[,1:K,drop=FALSE]%*%diag(sqrt(svd(tmp[[x]])$d[1:K]),nrow=K)), simplify = FALSE))
  
  # system.time(Ul<-mclapply(Nchunk_list, function(a) sapply(a, function(x) (svd(tmp[[x]])$u[,1:K,drop=FALSE]%*%diag(sqrt(svd(tmp[[x]])$d[1:K]),nrow=K)), simplify = FALSE)))
  # 
  # 
  # library(snow)
  # cl <- makeCluster( 5, type="MPI" )
  # clusterExport(cl, c('tmp'))
  # 
  # system.time(Ul<-parLapply( cl,Nchunk_list, function(a) sapply(a, function(x) (svd(tmp[[x]])$u[,1:K,drop=FALSE]%*%diag(sqrt(svd(tmp[[x]])$d[1:K]),nrow=K)), simplify = FALSE)))
  # 
  # stopCluster(cl)
  # mpi.exit()
  # 
...
#   colnames(TAC) <- names_i
  ...
  for (s in 1:(nscan + burn))
  {
    ...
    # system.time(tmp<-rbeta_a_fc_per_b(Fl,W=W,s2=s2,U=U,ivA=prior$ivA,beta0=prior$beta0,S0=prior$S0))
    # 
    # 
    # system.time(tmp<-sapply(Nchunk_list, function(x) rbeta_a_fc_per_b_chunk(x,Fl,W=W,s2=s2,U=U,ivA=prior$ivA,beta0=prior$beta0,S0=prior$S0)))
    # 
    # cl<- makeCluster (2, type='FORK',outfile="")
    # 
    # system.time(tmp<-parSapply(cl,Nchunk_list, function(x) rbeta_a_fc_per_b_chunk(x,Fl,W=W,s2=s2,U=U,ivA=prior$ivA,beta0=prior$beta0,S0=prior$S0)))
    # stopCluster(cl)
    # 
    # rbeta_a_fc_per_b_chunk(Fl,W=W,s2=s2,U=U,ivA=prior$ivA,beta0=prior$beta0,S0=prior$S0)
  ...
  fit <- list(BETAPM=BETAPM,GAMMAPM=GAMMAPM, COV=COV, APM=APM,bPM=bPM,U=U,UVPM.l=UVPM.l,THETAPM = THETAPM,UPM=UPM, ALPHAPM = ALPHAPM,EFlPM=EFlPM,ETPM=ETPM,
              X=X,Y=Y, Theta=Theta, Alpha=Alpha, b=b, input=input)
