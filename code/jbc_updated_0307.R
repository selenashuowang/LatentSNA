...
jbc_updated_0307<- function(X, Y,W, H, D=1,theoretical.str,
                    ...
                    prior=list())
{
  ...
  VR<-matrix(nrow=0,ncol=length(c(seq(1,(V)*(V+1)/2), seq(1,(D)*(D+1)/2))))

  COR<-matrix(nrow = 0, ncol = V*D)

  colnames(VR) <- c(paste("Ru",seq(1,(V)*(V+1)/2),sep=""),
                    paste("Rtheta",seq(1,(D)*(D+1)/2),sep=""))
  colnames(COR) <- paste("Rutheta",seq(1,(D)*V),sep="")
  ...
  for (s in 1:(nscan + burn))
  {
    ...
    tmp_alpha <- Procrustes(Alpha_tmp, theoretical.str,
                      translate = FALSE,
                      dilate = FALSE,
                      sumsq = FALSE)
    Alpha=Alpha_tmp %*% tmp_alpha$R
    Theta=Theta %*% tmp_alpha$R
...
#
    ...
    if(s%%odens==0 & s>burn)
    {
      ...
      Ru = matrix(COR_iter[1:V,1:V], nrow=V, ncol=V)
      Rtheta = matrix(COR_iter[(V+1):(V+D),(V+1):(V+D) ], nrow=D, ncol=D)
      Rutheta =matrix(COR_iter[(V+1):(V+D),1:V ], nrow = D, ncol = V)


      VR<-rbind(VR, c( Ru[upper.tri(Ru, diag = T)],
                       Rtheta[upper.tri(Rtheta, diag = T)]))
      COR<-rbind(COR, as.vector(Rutheta))
  ...
  fit <- list(BETAPM=BETAPM,GAMMAPM=GAMMAPM,VR=VR, COR=COR, VC=VC,COV=COV, APM=APM,bPM=bPM,U=U,UVPM.l=UVPM.l,THETAPM = THETAPM,UPM=UPM, ALPHAPM = ALPHAPM,EFlPM=EFlPM,ETPM=ETPM,
