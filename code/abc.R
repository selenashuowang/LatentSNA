...
#' \item{YPM}{mean of the generated data based on posterior samples} 
#' \item{X}{observed X} 
#' \item{Y}{observed Y} 
...
{ 
  ...
  { 
    ...
    # update gamma,  b
    tmp1=rgamma_b_fc(Tl,H=H,s1=s1,offset=Theta,ivB=prior$ivB,gamma0=prior$gamma0,S1=prior$S1)
    gamma <- tmp1$gamma
    b <- tmp1$b
    ...
    if (K > 0)
    {
      U<-rU_original(Fl,U,Theta, Stheta, Sutheta, Su, s2, offset=sapply(EFl, function(x) x-U%*%t(U), simplify = FALSE))
  ...
  rownames(Theta) <- rownames(U)<-rownames(X[[1]]) 
  ...
  fit <- list(BETAPM=BETAPM,GAMMAPM=GAMMAPM, VC=VC,COV=COV, APM=APM,BPM=BPM,U_1=U,U=U_1,UVPM=UVPM,THETAPM = THETAPM, EFlPM=EFlPM,ETlPM=ETlPM,
              XPM=XPM,YPM=YPM, Theta_1=Theta, GOF=GOF,X=X,Y=Y, UVC=UVC, TAC=TAC, input=input, indices=indices, indices_irt=indices_irt)
