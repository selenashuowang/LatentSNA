...
jbc_determinancy_0301<- function(X, Y,W, H, K=2,D=1,theoretical.str,
                    ...
                    prior=list())
{
  ...
  for (s in 1:(nscan + burn))
  {
    ...
    if (K > 0)
    {
      ...
      # for (n in 1:N){
      #   U[,,n] <- U[,,n] -   colMeans(U[,,n])
      #   #U[,,n] <- t(mhalf(solve(cov(U[,,n]))) %*% t(U[,,n]))
      #
      #
      # }
      #
      # for (u in 1:V){
      #   U[u,,] <- U[u,,] -   rowMeans(U[u,,])
      #   #U[u,,] <- mhalf(solve(cov(t(U[u,,])))) %*% U[u,,]
      #
      #
      # }


      # if(s ==1){U_target <- scale(U[,,1])}
      # if(s>1){
      #
      #   for (n in 1:N){
      #   tmp <- Procrustes(U[,,n], U_target,
      #                     translate = FALSE,
      #                     dilate = FALSE,
      #                     sumsq = FALSE)
      #   U[,,n]<-tmp$X.new
      ...
      #   U_target_1=tmp_a$scores$Y
      # }

      #if(s ==1){U_target_1 <- scale(t(U[1,,]))}
    #   if(s>1){
    #
    #     for (u in 1:V){
    #       tmp <- Procrustes(t(U[u,,]), U_target_1,
    #                         translate = FALSE,
    #                         dilate = FALSE,
    #                         sumsq = FALSE)
    #       U[u,,]<-t(tmp$X.new)
    ...
    # for(i in 1:3){
    #   Alpha[,i]=((1/apply(Alpha,2,prod))^(1/P))[i]*Alpha[,i]
    #
    # }
