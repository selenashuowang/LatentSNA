...
#'
#' @export rU
rU_b<-function(Fl,U,Theta, Stheta, Sutheta, Su, s2, offset=offset)
{
  ...
  for (n in 1:N){
    if(K==1){U[,,n]=matrix(sapply(1:V, function(x) rU_each_b(u=x, n, Es,U,Theta, invUTheta, invThetaU, ivU, to, K,N), simplify = TRUE))}
    if(K>1){U[,,n]=t(sapply(1:V, function(x) rU_each_b(u=x,n, Es,U,Theta, invUTheta, invThetaU, ivU, to, K,N), simplify = TRUE))}
  }
