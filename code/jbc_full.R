...
jbc_full<- function(X, Y,W, H, K=K,  D=1,theoretical.str,
...
if(K >0){
  if(is.null(prior$Su0)){ prior$Su0<-diag(K) }
  if(is.null(prior$etau)){ prior$etau<-(K+2) }
}
  ...
  for (s in 1:(nscan + burn))
  {
    ...
    s2<-rs2_fc_all(Fl,offset=EFl,nu2=prior$nu2,s20=prior$s20)
    #rs2(Fl,offset = EFl, nu2=prior$nu2,s20=prior$s20)
    s1<-rs1_b(Tm,offset = ET, nu1=prior$nu1,s10=prior$s10)



    tmp<-rbeta_a_fc_per_full(Fl,W=W,s2=s2,U=U,S=S,ivA=prior$ivA,beta0=prior$beta0,S0=prior$S0)
    beta <- tmp$beta
    ...
    if (K > 0)
    {
      ...
      tmp.S <- rSu(S, Su0=prior$Su0,etau=prior$etau)
      Sig_S=tmp.S$Su


      S<-rU_con(Fl,S, Sig_S, s2, offset=sapply(EFl, function(x) x-S%*%t(S), simplify = FALSE))
      S <- S -   colMeans(S)

      if(s ==1){S_target<-S}

      if(s>1){
        tmp <- Procrustes(S, S_target,
                          translate = FALSE,
                          dilate = FALSE,
                          sumsq = FALSE)
        S<-tmp$X.new
  ...
  fit <- list(BETAPM=BETAPM,GAMMAPM=GAMMAPM, SSPM=SSPM, SPM=SPM,VC=VC,COV=COV, APM=APM,bPM=bPM,U=U,UVPM.l=UVPM.l,THETAPM = THETAPM,UPM=UPM, ALPHAPM = ALPHAPM,EFlPM=EFlPM,ETPM=ETPM,
