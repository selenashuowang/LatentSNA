...
jbc_scaleAlpha_new_abcd_loop<- function(W, H, D=1,theoretical.str,family_behavior,
                    indices = NULL, indices_irt = NULL,
                    seed = 1, nscan = 10000, burn = 500, odens = 25,
                    print = TRUE, gof=TRUE, plot=TRUE,
                    prior=list(), res=model1,name=string_list)
{
  ## record model set up
  ...
  if(substring(name[[1]][[1]],1,1)=="s"){family_behavior="nrm"}
  if(substring(name[[1]][[1]],1,1)=="r"){family_behavior="bin"}
  ...
  ## behavior parameters
  bPS<- res$bPM*nrow(res$COV)
  ALPHATHETAPS <- (res$ETPM - matrix(rep(1,nrow(Y))) %*% t(res$bPM))*nrow(res$COV)
  ALPHAPS <- res$ALPHAPM*nrow(res$COV)
  THETAPS <- res$THETAPM*nrow(res$COV)
  UPS <-res$UPM*nrow(res$COV)
  
  EFlPM<-res$EFlPM
  ETPM<-res$ETPM

  UVPS.l=res$UVPM.l
  APS<-res$APM*nrow(res$COV)
  ...
  fit <- list(BETAPM=BETAPM,GAMMAPM=GAMMAPM, COV=COV, APM=APM,bPM=bPM,U=U,UVPM.l=UVPM.l,THETAPM = THETAPM,UPM=UPM, ALPHAPM = ALPHAPM,EFlPM=EFlPM,ETPM=ETPM,
              X=X,Y=Y, Theta=Theta, Alpha=Alpha, b=b, input=input, U_target=U_target)
